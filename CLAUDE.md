...
**`src/config.py`** — `PipelineConfig(BaseSettings)` from `pydantic-settings`. Single source of truth for all runtime configuration: LLM/API credentials, generation params (max_tokens, temperatures, row caps), DB paths, schema settings, and all feature flags with their retry budgets. Reads from `.env` file and OS environment variables; constructor kwargs take highest priority. `LOG_LEVEL` / `LOG_FORMAT` are the only env vars read directly (in `logging_config.py`) because `configure_logging` runs before any `PipelineConfig` is instantiated. `build_default_llm_client(config)` is the factory used by `AnalyticsPipeline`.
